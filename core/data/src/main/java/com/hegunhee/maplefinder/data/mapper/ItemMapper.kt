...
import com.hegunhee.maplefinder.model.character.item.normal.Title
...
internal fun ItemEquipmentResponse.toModel(): Item {
    val scrollOption = ScrollOption(
        ...
        scrollRecoverableCount,
        goldenHammerFlag
    )
    val isStarforceItem = isStarforceItemSlot(slot)
    val starCountMax = starMaxCount(baseOption.baseLevel)
    val starforceOption =
        StarforceOption(isStarforceItem, starCountMax, starforce, starforceScrollFlag)
    return Item(
        part = part,
        slot = slot,
        ...
        potentialOption = cubeOptionToModel(
            potentialOptionGrade,
            potentialOption1,
            potentialOption2,
            potentialOption3
        ),
        additionalOption = cubeOptionToModel(
